...
import com.soybeany.bdlib.basic.widget.output.listview.simple.SimpleRecyclerViewHelper;

/**
 * 带提示的重用视图辅助器，继承自{@link SimpleRecyclerViewHelper}，添加了提示语功能
 * <br>注意：recyclerView需先添加到某个viewGroup中
 * <br>Created by Soybeany on 2017/3/18.
 */
public class HintRecyclerViewHelper<Data> extends SimpleRecyclerViewHelper<Data> implements IHintListView, HintListViewImpl.IHintListListener {

    private HintListViewImpl mHintListViewImpl; // 提示语处理者

    public HintRecyclerViewHelper(RecyclerView recyclerView, IItemFactory<Data> factory) {
        this(recyclerView, factory, null, null);
    }

    public HintRecyclerViewHelper(RecyclerView recyclerView, IItemFactory<Data> factory, IOnRefreshListener refreshListener, IOnLoadListener loadListener) {
        super(recyclerView, factory, refreshListener, loadListener);
        mHintListViewImpl = new HintListViewImpl(this);
        mHintListViewImpl.transfer(factory.getContext(), recyclerView);
    }
