...
public class EmergencyHandler implements Thread.UncaughtExceptionHandler {
     ...
     */
    public static void init(String path, boolean addSdPrefix, ICallback callback) {
        PATH = addSdPrefix ? (FileUtils.SD_PATH + path) : path;
    ...
    private void processException(final Throwable th) {
        try {
            ...
            FileUtils.saveString(sb, new File(PATH, fileName), false);
